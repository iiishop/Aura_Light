...
11. If you want to use some morden method, you can use the Dashboard in this project to control the device, by a http server running on your computer. You have to modify the `broker` `port` `username` `password` variables in the `config.js` file in the `Dashboard` folder to match your MQTT server information. Then open the `index.html` file in the `Dashboard` folder in your web browser.
